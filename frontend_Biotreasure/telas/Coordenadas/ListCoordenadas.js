...
const ListCoordenadas = ({ navigation }) => {
  ...
  const fetchCoordenadas = async () => {
    try {
      const data = await lerCoordenadas(); // Corrige a variável de retorno para `data`
      console.log('Dados recebidos:', data); // Log para verificar os dados recebidos
      setCoordenadas(data || []); // Ajusta para garantir que é uma lista
    } catch (error) {
      console.error('Erro ao buscar coordenadas:', error);
      Alert.alert('Erro', 'Falha ao buscar coordenadas.'); // Notifica o usuário
    }
  };
        ...
        <FlatList
          ...
          renderItem={({ item: coordenada }) => (
            <View style={styles.itemContainer}>
              ...
              <Text style={styles.itemText}>Animal ID: {coordenada.animal_id}</Text>
